...
def register_timeline_operations(mcp, resolve: Optional[object]):
    """Register timeline operations with the MCP server."""
    
    @mcp.resource("resolve://timelines")
    def list_timelines_resource() -> List[str]:
        """List all timelines in the current project."""
        return list_timelines(resolve)
    
    @mcp.resource("resolve://current-timeline")
    def get_current_timeline_resource() -> Dict[str, Any]:
        """Get information about the current timeline."""
        return get_current_timeline_info(resolve)
    
    @mcp.tool()
    def create_timeline_tool(name: str) -> str:
        """Create a new timeline."""
        return create_timeline(resolve, name)
    
    @mcp.tool()
    def set_current_timeline_tool(name: str) -> str:
        """Set the current timeline by name."""
        return set_current_timeline(resolve, name)
    
    @mcp.tool()
    def add_timeline_marker_tool(frame: int, color: str = "Red", name: str = "", note: str = "") -> str:
        """Add a marker to the current timeline."""
        return add_timeline_marker(resolve, frame, color, name, note)
...
def set_current_timeline(resolve, name: str) -> str:
    ...
    if not target_timeline:
        available_timelines = [current_project.GetTimelineByIndex(i).GetName() 
                             for i in range(1, timeline_count + 1) 
                             if current_project.GetTimelineByIndex(i)]
        return f"Error: Timeline '{name}' not found. Available timelines: {', '.join(available_timelines)}"
    
    result = current_project.SetCurrentTimeline(target_timeline)
    if result:
        return f"Successfully switched to timeline '{name}'"
    else:
        return f"Failed to switch to timeline '{name}'"

def add_timeline_marker(resolve, frame: int, color: str = "Red", name: str = "", note: str = "") -> str:
    """Add a marker to the current timeline."""
    if resolve is None:
        return "Error: Not connected to DaVinci Resolve"
    ...
    # Validate frame is within timeline bounds
    start_frame = current_timeline.GetStartFrame()
    end_frame = current_timeline.GetEndFrame()
    
    if frame < start_frame or frame > end_frame:
        return f"Error: Frame {frame} is outside timeline bounds ({start_frame} to {end_frame})"
    
    # Validate color
    valid_colors = ["Blue", "Cyan", "Green", "Yellow", "Red", "Pink", "Purple", "Fuchsia", "Rose", "Lavender", "Sky", "Mint", "Lemon", "Sand", "Cocoa", "Cream"]
    if color not in valid_colors:
        return f"Error: Invalid color '{color}'. Valid colors: {', '.join(valid_colors)}"
    ...
    result = current_timeline.AddMarker(frame, marker_data)
    if result:
